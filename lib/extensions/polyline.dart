...
/// Encodes `Iterable<LatLng>` of [coordinates] into a `String` via
...
String encodePolyline(Iterable<LatLng> coordinates, {int accuracyExponent = 5}) {
  assert(() {
    if (accuracyExponent < 1) {
      throw ArgumentError.value(
        accuracyExponent,
  ...
  for (var i = 1; i < coordinates.length; i++) {
    polyline += encodePoint(
      coordinates.elementAt(i).latitude,
      previous: coordinates.elementAt(i - 1).latitude,
      accuracyExponent: accuracyExponent,
    );
    polyline += encodePoint(
      coordinates.elementAt(i).longitude,
      previous: coordinates.elementAt(i - 1).longitude,
      accuracyExponent: accuracyExponent,
    );
  }
...
Iterable<LatLng> decodePolyline(String polyline, {int accuracyExponent = 5}) {
  ...
  while (index < polyline.length) {
    ...
    coordinates.add(
      LatLng(
        lat / accuracyMultiplier,
        lng / accuracyMultiplier,
      ),
    );
  }
